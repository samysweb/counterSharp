...
class CondCheckTransformVisitor(TransformVisitor):
	...
	def visit_FuncCall(self, node, parents):
		...
		return self.buildCheck(node)

	def buildCheck(self, node):
		tCompound = c_ast.Compound([
			c_ast.Assignment('=',
			...
			c_ast.Goto(self.config.gotoLabel)
		])
		ifStmt = c_ast.If(node.args.exprs[0],tCompound,c_ast.EmptyStatement(coord=CondCheckTransformVisitor.TransformCoord))
		return ifStmt